...
MarkovNode *get_first_random_node (MarkovChain *markov_chain)
{
  ...
  while (ends_with_point (linked_list_ind (linked_data, rand_ind)->data->data)
         == TRUE)
    {
          ...
          if (first->size_of_next_array != 0)
            {
              ...
              if (ends_with_point (next_node->data) == TRUE)
                {
...
{
  Node *tmp = (*ptr_chain)->database->first;
  while (tmp != NULL)
    {
      free (tmp->data->data);
...
{
  char *buffer = malloc (strlen (data_ptr) + 1);
  if (buffer == NULL)
    {
  ...
  if (node_to_check == NULL)
    {
      ...
      if (add (markov_chain->database, new_node) == 0)
        {
          if (allocate_next_states_array (new_node)
              == TRUE)
            {
